...
namespace triton {
  namespace engines {
    namespace symbolic {
      ...
      void PathConstraint::addBranchConstraint(bool taken, triton::uint64 srcAddr, triton::uint64 dstAddr, std::shared_ptr<triton::ast::AbstractNode> const& pc) {
        if (pc == nullptr)
      ...
      const std::vector<std::tuple<bool, triton::uint64, triton::uint64, std::shared_ptr<triton::ast::AbstractNode>>>& PathConstraint::getBranchConstraints(void) const {
      ...
      std::shared_ptr<triton::ast::AbstractNode> PathConstraint::getTakenPathConstraintAst(void) const {
        for (auto it = this->branches.begin(); it != this->branches.end(); it++) {
          if (std::get<0>(*it) == true)
            return std::get<3>(*it);
        }
